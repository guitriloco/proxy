"""
API FastAPI - Proxy Aggregator
Serve proxies vivos via REST API com atualização automática.
"""
import asyncio
...
from src.models.proxy import Proxy, ProxyStatus, Protocol


# Estado global
PROXIES_VIVOS: list[Proxy] = []
scheduler = AsyncIOScheduler()


async def coletar_e_validar():
    """Coleta proxies das fontes e valida"""
    global PROXIES_VIVOS
    
    print("[*] Coletando proxies das fontes...")
    proxies = await collect_all()
    print(f"[*] {len(proxies)} proxies coletados, validando...")
...
app = FastAPI(
    title="Proxy Aggregator API",
    description="API para servir proxies vivos",
    version="1.0.0",
    lifespan=lifespan
)
...
async def get_proxies(
    ...
    """Retorna lista de proxies vivos"""
